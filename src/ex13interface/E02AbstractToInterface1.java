...
VO(Value Object)로 표현하기도한다. 해당 프로그램에서는 이름과 주민번호를 저장하는 용도로 사용된다.
*/
class PersonalInfoDTO {
	...
	//getter/setter: DTO객체에서 private으로 선언된 멤버변수의 값을 설정하거나 반환활때 사용한다.
	...
	public String searchPersonalInfo(String juminNum) {
		...
		for(int i=0 ; i<numOfPerInfo ; i++) {
			...
			if(juminNum.compareTo(personalArr[i].getJuminNum())==0) {
				...
				/*
				DTO객체의 멤버변수는 private으로 선언되었으므로 외부에서 접근이 불가능하다.
