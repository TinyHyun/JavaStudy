...
추상메서드
- 오버라이딩을 목적으로 제작된다.
- 메서드의 실행부가 없으므로 중괄호 대신 ;으로 마무리한다.
*/
...
/*
추상메서드를 포함하지 않은 추상크래스를 상속하면 오버라이딩은 강제사항이 아닌 선택사항이다.
...
/*
위 클래스를 상속하면 부모가 가진 추상메서드를 그래도 상속받게 되므로 해당 클래스도 abstract로 선언해야한다.
단, 이 경우 해당 클래스로도 인스턴스를 생성할 수 없게된다.
따라서 객체생성을 위해서는 부모 추상메서드를 자식쪽에서 오버라이딩 하면된다.
*/
...
public class E01AbstractMain {
	...
	public static void main(String[] args) {
		...
		//AbsClass1 absClass1 = new AbsClass1(); => 추상클래스는 인스턴스를 생성할 수 없다.
		
		//추상클래스를 상속한 하위클래스는 인스턴스 생성을 할 수 있다.
		AbsClassChild1 absClassChild1 = new AbsClassChild1();

		//추상클래스는 인스턴스 생성은 할 수 없지만 참보변수로는 사용할 수 있다.
		...
		//추상클래스내에 선언된 정적 멤버메서드는 오버라이딩 대상이 될수 없으므로 static의 기본규칙을 따른다.
