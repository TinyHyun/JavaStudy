...
class EncapsulLogic {
	...
	MemberRegist memberRegist = new MemberRegist(); //가입
	FirstLoginEvent firstLoginEvent = new FirstLoginEvent(); //이벤트페이지
	AutoLogin autoLogin = new AutoLogin(); //자동로그인
	CongratulationPoint congratulationPoint = new CongratulationPoint(); //포인트 지급
	...
	void doProcess() {
		...
		firstLoginEvent.doFirstLoginEvent(); //이벤트페이지
	}
}
...
public class E05Encapsulation {
	...
	public static void main(String[] args) {
		...
		/*
		업무의 진행순서가 중요하다고 가정했을 때 캡슐화 이전의 코드에서는 개발자가 순서를 지키지 않은
