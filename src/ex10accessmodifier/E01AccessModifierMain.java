...
public class E01AccessModifierMain {
	...
	public static void main(String[] args) {
		...
		//동일한 패키지에 선언된 클래스이므로 import 없이 인스턴스를 생설할 수 있다.
...
//		one.privateMethod(); => 에러발생
		...
		/*
		private으로 선언된 멤버에 외부클래스에서 접근하려면 public으로 선언된 멤버메서드를 통해
		간접적으로 접근해야한다. 클래스 내부에서는 접근지정자의 영향을 받지 않으므로 해당 메서드를
		통해 접근하는것이 가능해지게 된다.
		*/
		one.publicMethod();
		...
		System.out.println("DefaultClsss 객체 생성 및 접근");
...
//		two.defaultMethod(); => 에러발생
		...
		//import자체가 불가능하므로 아예 사용할 수 없는 클래스가된다.
		System.out.println("DefaultClsss 객체 생성 및 접근");
