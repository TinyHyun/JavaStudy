...
import common.Student; // => 패키지가 다르므오 import된다.
...
//		ArrayList<Student> list2 = new ArrayList<Student>();
		...
		//저장할 Student인스턴스 생성. 외부패키지에 선언되었으므로 반드시 import후 사용해야 한다.
		Student st1 = new Student("정우성", 10, "2018");
		Student st2 = new Student("원빈", 20, "2017");
		Student st3 = new Student("장동권", 30, "2016");
		...
		/*
		일반 for문 사용을 위해 size() 메서드로 추가된인스턴스의 갯수를 반환받는다.
		...
		*/
		...
		즉, 동시에 삭제되지 않는다.
		*/
		boolean removeOk = list2.remove(st2); //원빈 삭제 => 앞에 있던 원빈이 삭제된다.
		//정삭적으로 삭제되었다면 true를 반환한다.
		...
		*/
		...
		for (Student st : list2) {
			...
			삭제를 위해 입력한 이름과 Student객체에 저장된 name이 일치하는지 확인한 후 해당 인스턴스를 찾을 수 있다.
			*/
