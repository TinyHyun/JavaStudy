...
Dead상태가 되어야만 실행이 종료되는 쓰레드를 말한다. 해당 쓰레드는 main() 메서드가 종료된 이후에도 계속 실행된다.
*/
class NormalThread extends Thread {
	...
	/*
	Thread클래스의 생성자 호출 시 전달하는 인수를 통해 해당 쓰레드의 이름을 지정할 수 있다.
	...
	*/
	...
	/*
	쓰레드 동작 중 비정상적으로 종료될 때 InterruptedException이 발생할 수 있으므로 예외처리를 해야한다.
	여기서는 즉시 처리하지 않고 throws해서 호출한 위치로 예외객체를 던지기한다.
	*/
...
class DaemonThread extends Thread {
	...
	/*
	무한루프로 구성되어있고 별도의 탈출 조건을 만들지 않는다.
	...
	public void run() {
		...
		while(true) {
			...
			try {
				...
				System.out.println("자동저장시 오류발생 ㅜ");
			}
		}
	}
}
...
public class Ex03DaemonThread {
	...
	public static void main(String[] args) {
		...
		/*
		쓰레드로 구현하지 않는 일반적인 클래스의 작업으로 순차적으로 진행되어
		1번 인스턴스가 작업을 마쳐야만 2번째가 실행된다.
		*/
		NormalClass nt1 = new NormalClass("첫번째 클래스");
		nt1.classMethod();
		new NormalClass("두번째 클레스").classMethod();
		...
		*/
		...
		//Thread클래스의 정적 메서드를 통해 쓰레드의 여러상태를 확인할 수 있다.
