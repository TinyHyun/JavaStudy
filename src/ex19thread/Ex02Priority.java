...
class MessageSendingThread extends Thread {
	...
	public void run() {
		//각 쓰레드는 1000번씩 반복출력한다. 이때 부여받은 우선 순위를 출력한다.
		for(int i=0 ; i<=1000 ; i++) {
			System.out.println(message + i + "("+ getPriority() + ")");
			/*
			기본적으로 우선순위가 높은 쓰레드가 먼저 실행되지만 sleep()을 호출하면 실행 중
			잠깐씩 블럭상태로 전환되어 우선순위가 낮은 쓰레드도 실행될 수 있는 기회가 생기게된다.
			*/
			try {
				sleep(1);
			}
...
public class Ex02Priority {
	...
	public static void main(String[] args) {
		...
		/*
		1단계 실행
		: 우선순위를 부여라지 않은 상태로 쓰레드 인스턴스를 생성하면 동일한 우선순위 5를 부여받게된다.
