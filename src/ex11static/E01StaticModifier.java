...
- 메서드 앞에 static을 붙이면 정적메서드가 된다. 정적변수와 모든 특성이 동일한다.
*/
...
class MyStatic {
	...
	*/
	...
	정적멤버변수: 객체를 생성할 필요없이 클래스명으로 바로 접근할 수 있다.
				JVM(자바가상머신)이 프로그램을 시작할 때 이미 메모리(Method 영역)에 로드된다.
	*/
	static int staticVar;
	
	/*
	인스턴스형 멤버메서드: 인스턴스형 메서드에서는 모든 멤버를 사용할 수 있다.
					인스턴스형 멤버, 정적 멤버 둘다 접근이 가능하다.
	*/
	void instanceMethod() {
	...
	정적멤버메서드: 정적메서드 내부에서는 인스턴스형 멤버에는 접근할 수 없다.
				정적 멤버만 접근할 수 있다.(※ main메서드에서는 static으로 선언된 메서드만 호출할 수 있다.)
	*/
...
public class E01StaticModifier {
	...
	public static void main(String[] args) {
		...
		//일반적인 멤버변수에 접근하는 방법: MyStatic클래스의 인스턴스를 생성한 후 참조변수를 통해 접근한다.
