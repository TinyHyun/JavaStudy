...
할당한 후 이를 프로그램 전체에서 공유해서 사용하고자할 때 쓰는 패턴이다.
...
5. 이 방법을 통해 인스턴스를 생성하면 메모리에 딱 하나만 만들어지게 되어 메모리를 절약할 수 있다.
*/
...
class NoSingleTone {
	...
	만약 public으로 선언하지 않으면 클래스 외부 혹은 다른 패키지에서는 호출할 수 없어
...
class SingleTone {
	...
	프로그램이 시작될 때 미리 메모리(Method)에 로드되어 사용할 준비를 마치게된다.
	*/
	private static SingleTone single = new SingleTone();
	
	/*
	생성자를 private으로 선언하면 클래스 외부에서 호출할 수 없으므로
	new를 통해 인스턴스를 생성할 수 없게된다.
	
	이때 생성자 메서드를 private으로 선언하지않고 생략하며 public으로 자동생성되기때문에
	new를 통해 인스턴스를 생성할 수 있게된다.
	*/
	private SingleTone() {}
...
public class E03SingleToneDesignPattern {
	...
	public static void main(String[] args) {
		...
		/*
		일반적인 방식의 인스턴스 생성이므로 new로 생성할 때마다
...
//		SingleTone st1 = new SingleTone(); // => 에러발생
		...
		*/
		...
		System.out.println(String.format("str2의 주소: %s", st2));
		System.out.println(String.format("str3의 주소: %s", st3));
		
		//결국 동일한 인스턴스를 사용한 것이므로 마지막에 할당된 값이 출력된다.
		System.out.println(String.format("str2의 shareVar: %d", st2.shareVar)); //200
		System.out.println(String.format("str3의 shareVar: %d", st3.shareVar)); //200
	}
