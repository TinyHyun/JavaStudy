...
/*
추상메서드를 포한한 인터페이스를 구현하면 하위 클래스에서는 반드시 오버라이딩 해야한다.
추상메서드를 포함한 클래스는 반드시 abstract로 선언하거나 오버라이딩을 통해 부모 인터페이스의 추상메서드를 재정의해야한다.
이렇게 가려지게 된다.
*/
...
public class Ex01NormalClass {
	...
	여기서는 부모타입의 참조변수를 통해 객체를 생성한 후 오버라이딩된 자식쪽의 메서드를 호출하고 있다.
	*/
