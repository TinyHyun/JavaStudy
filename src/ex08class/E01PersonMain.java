...
또한 public 접급지정자는 하나의 Java파일에 한번만 사용할 수 있다. 
*/
class Person{
	...
	*/
	...
	멤버메서드 : 클래스에서 객체의 동작(행위)를 표현한다.
	 	클래스 외부에서 호출시에는 객체의 참조변수를 사용해서 .으로 호출한다.
	 	또한 멤버변수는 멤버메서드 내에서 즉시 사용할 수 있다.
	
	※ 넓은지역(클래스)에서 생성된 멤버변수는 좁은지역(멤버메서드)에서 사용할 수 있으므로 별도의 전달없이
	  아래와 같이 출력문에서 사용할 수 있다. 
	*/
