...
class Car{
	...
	/*
	우리가 직접 작성한 클래스느 객체를 표현하기 위한 용도로 생성하지만
	...
	*/
	...
	/*
	초기화메서드1 : 매개변수가 없는 형태로 항상 고정된 값으로만 인스턴스를 초기화 할 수 있다.
	...
	*/
	...
	/*
	초기화메서드2 : 위와 동일한 이름의 메서드로 <<오버로딩>>에 의해 정의되어있다.
		초기화를 진행하는 역할은 동일하나 매개변수를 통해 다양한 값을 초기화할 수 있다.
	*/
	...
	void showCarInfo(){
		...
		//모델명은 Car클래스의 멤버변수이므로 그래도 사용한다.
		System.out.printf("모델명:%s\n", carModel);
		//소유주를 표현한 owner객체의 멤버메서드 호출을 통해 정보를 출력할 수 있다. 이름, 나이 등이 출력된다.
		owner.showState();
	}
}///end of Car Class
...
public class E03CarMain {
	...
	public static void main(String[] args) {
		...
		//자동차1 객체 생성
		//매개변수가 없는 초기화메서드를 호출하여 모델명을 람보르기니로 초기화한다. 
		Car car1 = new Car();
		car1.initialize();
		car1.showCarInfo();
		...
		//자동차4 객체 생성
		//해당 인스턴스는 생성만 하고 초기화하지 않은 상태로 출력을 시도하므로 예외가 발생한다.
		//즉 객체를 초기화 진행한 후 사용해야한다.
		Car car4 = new Car();
		car4.showCarInfo();
	}
