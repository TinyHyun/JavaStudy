...
public class Human {
	...
	int age;
	//에너지는 증가 혹은 감소가 필요하므로 반드시 int로 설정해야한다.
	int energy;
	
	//멤버메서드 : 객체의 행위을 표현한다.
	void walking() { // '걷는다'를 표현
		/*
		걷는 행위는 에너지를 1씩 감소시킨다.
		만약 에너지가 감소된 결과가 음수라면 해당 시나리오의 조건에 위배되므로 0으로 고정하는 코드가 포함되어있다. 
		 */
		energy--;
		if(energy<0) {
			//감소 후 음수가되면 0으로 고정한다.
