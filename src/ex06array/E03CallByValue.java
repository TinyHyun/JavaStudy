...
: 메서드 호출과 관련된 개념으로 매개변수를 통해 값을 전달할 때 메모리의 '복사'를 통해 값이 전달된다.
 ...
 메모리적 관점에서 보면 스택영역만 사용하는 케이스로 main과 호출된 메서드는 서로 다른 지역이므로 서로에게 전혀
 영향을 미치지 않는것이 특징이다.
*/
public class E03CallByValue {
	...
	public static void main(String[] args) {
		...
		//매개변수로 전달된 값은 복사되어 다른 지역의 메서드 내에서만 변경되므로 main메서드에서는 초기값이 그래도 유지된다. 
	...
	public static void callByValue(int fNum, int sNum) {
		...
		swap(교환) : 두개의 변수가 할당받은 값을 서로 교환하는 것을 말한다.
			교환시에는 임시변수(여기서는 temp)가 하나 추가로 필요하다.
		*/
		int temp;
		temp = fNum;
		fNum = sNum;
