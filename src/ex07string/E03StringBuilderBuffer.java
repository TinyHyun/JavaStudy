...
기존 메모리가 소멸되고 새로은 메모리가 생성되는 낭비를 막기위해, 문자열의 변경이 많은 경우에는
StringBuffer 클래스를 사용하는것이 좋다. 해당 클래스는 기존 메모리의 문자열을 변경하는 방식으로 추가한다.
*/
...
public class E03StringBuilderBuffer {
	...
	public static void main(String[] args) {
		...
		*/
		...
		//문자열의 길이를 삽입위치로 지정하면 문자열의 끝에 추가된다.
		//문자열의 길이를 통해 마지막 부분에 'Z'를 추가한다.
		strBuf.insert(strBuf.length(), 'Z');
		System.out.println("strBuf= "+ strBuf);
		
		System.out.println("String과 StringBuffer의 "+"참조값 비교");
		
		//""을 사용해서 동일한 문자열 인스턴스를 생성했으므로 동일한 주소값을 가진다.
		...
		/*
		StringBuffer클래스는 문자열을 저장하기 위해 기본 메모리를 16으로 할당한다.
