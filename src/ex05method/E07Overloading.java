...
- 컴파일러는 메서드 호출시 전달되는 인수(파라미터, 전달되는 값)를 통해 호출 할 메서드를 구분한다.
*/
...
public class E07Overloading {
	...
	/*
	해당 메서드는 앞에서 선언한 것과 반환타입만 다른 경우이므로 person(10)으로 호출한다 가정했을 때 어떤 메서드를
	호출할지 애매하다. 컴파일러는 애매한 경우 에러를 발생시킨다.
	따라서 아래의 메서드는 오버로딩이 성립되지 않는다. 
	*/
