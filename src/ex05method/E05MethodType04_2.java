...
public class E05MethodType04_2 {
	...
	/*
	main()에서 잔달한 인수의 크기만큼 반복해서 정수를 입력받는다.
	그 중에 최대값을 찾아서 반환한다. 
	*/
	static int returnMaxNumber(int numberCnt) {
		...
		for(int i=1 ; i<=numberCnt ; i++) {
			...
			if(i==1) {
				//입력받은 정수중에서 최대값을 찾아야 하므로, 첫번째 입력값을 비교를 위한 기준값을 설정한다. 
				//즉, 비교의 대상이 없으므로 기준값이 무조건 최대값이 된다.
				maxVal = inputNum;
			}
			else {
				//두번째 입력값부터는 기존의 최대값과 비교해서 더 큰값이 발견되면 최대값을 교체한다.
				if(maxVal<inputNum) {
					maxVal = inputNum;
				}
			}
		}
		//새로운 입력값이 기존값보다 크다면 지속적으로 교체하여 가장 큰 값을 호출한 지점으로 반환한다.
		return maxVal;
	}
