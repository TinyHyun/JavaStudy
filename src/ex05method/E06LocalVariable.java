...
/*
지역변수(Local variable)
- 변수는 사용범위(Scope)를 가지고 있고, 해당 지역내에서만 사용할 수 있따.
- 해당 지역을 벗어나면 즉시 메모리에서 소멸된다.
- 기본자료형은 스택이라는 메모리를 사용한다.
...
public class E06LocalVariable {
	...
	public static void main(String[] args) {
		...
		*/
//		int num = 9; => 주석을 풀면 아래 코드에서 에러가 발생한다.
		...
		*/
		...
		// 위에서의 num은 if~else문이 끝나면서 메모리에서 소멸된다.
		
		// 따라서 새로운 num을 main지역에 생성할 수 있다.
		int num = 100;
	...
	/*
	main에서 선언한 변수 num은 메모리에 아직 상주하고 있지만 해당 메서드는 크기가 동일하지만 서로 다른 지역에 정의되었으므로
	같은 이름의 변수를 선언할 수 있다. 앞의 main메서드의 if~else문의 블럭도 같은 개념으로 변수를 선언한 것이다. 
	*/
	static void simpleFunc() {
		
		//main지역의 바깥에 새로운 지역이 만들어져서 num울 생성할 수 있다.
		int num = 1000;
		System.out.println("simpleFunc 메서드 지역: "+ num);
	}
	//호출한 지점으로 돌아간 후 소멸된다.
