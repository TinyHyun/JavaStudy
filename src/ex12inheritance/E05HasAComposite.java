...
여기서는 권종을 소유한 경찰을 표현한다.

Has-A와 같이 소유의 관계를 상속으로 표현하면 클래스간의 강한 연결고리가 형성되어 권총이 없는 경찰을 
...
class Police {
	...
	public Police(int bNum, int hCuff) {
		...
		즉, Heap영역에 생성된 인스턴스가 아예 없으므로 소유하지 않은 것을 표현할 수 있다.
		*/
		if(bNum<=0) {
			gun = null;
		}
