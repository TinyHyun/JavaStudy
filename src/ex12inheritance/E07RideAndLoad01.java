...
오버라이딩: 메서드명, 매개변수, 반환형까지 모두 동일한 메서드를 상속관계에 있는 하위클래스에서 재정의 한 것을 말한다.
...
class C extends B {
	...
	멤버메서드와 반환혀으 메서드명, 매개변수의 갯수 및 타입이 모두 동일하다.
	즉, 똑같은 메서드이다.
	*/
	@Override
	public void rideMethod() {
	...
	*/
	...
	/*
	따라서 C클래스로 인스턴스를 생성하면 오버라이딩된 메서드는 모두 가려지게되므로
	C클래스에 정의된 메서드 하나만 남게된다. 오버로딩 된 메서드는 서로 다르므로 3개 모두 존재한다.
	*/
}
...
//		 ref1.loadMethod(1.1); => 에러발생
		 ...
		 */
		 ...
		 C refAddr1 = new C();
		 //B타입이 C타입을 참조하므로 가능
		 B refAddr2 = refAddr1; //=> C
		 //A타입이 C타입을 참조하므로 가능
		 A refAddr3 = refAddr1; //=> C
...
//		 C refId3 = refId1; //=> A
		 ...
		 /*
		 자식으로 부모인스턴스를 참조하는 것은 기본적으로 불가능 하다.
