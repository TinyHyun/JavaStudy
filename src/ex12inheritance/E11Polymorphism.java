...
/*
다형성(Polymorphism)
: 하나의 인스턴스(객체)를 여러타입의 참조변수를 통해 참조할 때 참조변수의 타입에 따라
...
class Child extends Parent {
	...
	*/
	void parentMethod(int childMember) {
		...
		System.out.println("Overloading:자식에서 확장된 메소드" + ": parentMethod(int childMember)");
	}
...
public class E11Polymorphism {
	...
	public static void main(String[] args) {
		...
		System.out.println("[자식으로 자식객체 참조 - 동일화]");
...
//		parent1.childMember = 1; => 부모타입으로 자식인스턴스에 접근할 수 없으므로 에러발생
		...
		하지만 아래와 같이 가술하면 에러가 발생한다.
		(Child)parent2.childMember
		소괄호를 하나 더 추가해서 parent2를 자식타입의 형변환 후 멤버변수에 접근해야 한다.
		*/
		...
		*/
		...
		System.out.println("[클래스의 끝판왕 Obuect]");
		/*
		Java에서 생성한 모든 클래스는 Object클래스를 상속한다.
		따라서 모든 인스턴스는 Object클래스에 정의된 모든 메서드를 별도의 선언없이 사용할 수 있고, 
		필요하다면 오버라이딩도 할 수 있다.
		*/
