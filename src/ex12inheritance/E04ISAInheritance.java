...
class Computer{
	...
	public으로 선언된 메서드를 간접적으로 호출해야한다.
	*/
...
class NotebookComputer extends Computer{
	...
	//생성자 메서드 정의
	public NotebookComputer(String name, int initCarge) {
		/*
		부모클래스의 생성자를 호출하기 위해 사용하느 super()는 생략할 수 있지만,
	...
	public void movigCal() {
		//사용전 배터리를 확인 후 사용여부를 판단한다.
		if(battary < 1) {
			System.out.println("배터리가 방전되어 사용불가");
			//메서드에서 아래와 같이 return을 만나서 실행이 중지된다.
			return; //=> 반환되는것은 없기때문에 여기서는 중지
		}
		System.out.print("이동하면서 ");
		
		//private으로 선언되었으므로 상속을 받은 자식클래스에서도 접근할 수 없다.
		//호출하고 싶다면 public으로 선언해야한다.
//		keyboardTouch(); => 에러발생 private으로 선언됨
//		calculate(); => 에러발생 private으로 선언됨
		calculateFunc();
...
/*
태블릿 노트북
: 컴퓨터의 기본 연산기능과 휴대이용 그리고 전용 펜으로 드로잉 할 수 있는 기능까지 추가된 컴퓨터를 추상화한다.
*/
class TabletNotbook extends NotebookComputer{
	...
	//생성자
	public TabletNotbook(String name, int initCharge, String pen) {
	...
	public void write(String penInfo) {
		...
		A.compareTo(B)
		: 문자열 A와 B를 비교하여 0이 반환되면 동일한 문자열이고 -1 혹은 1이 반환되면 서로 다른
		문자열로 판단하는 String클래스에 정의 된 메서드
		*/
		...
		단, 접근이 가능한 지정자로 선언되어야 한다. super는 생략할 수 있다.
		*/
		super.movigCal();
...
public class E04ISAInheritance {
	...
	public static void main(String[] args) {
		...
		//테블릿 인스턴스 생성
		TabletNotbook tablet = new TabletNotbook("이동욱", 5, "ISE-1234");
