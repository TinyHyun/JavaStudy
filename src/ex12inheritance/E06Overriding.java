...
메서드를 재정의하는 것을 말한다. 오버로딩과 다른점은 매개변수의 갯수 및 타입, 반환타입 등이
완전히 똑같은 형태로 정의하는 것이다. 하위클래스에서 오버라이딩한 상위클래스의 메서드를 가리게 되므로
...
class BaseSpeaker extends Speaker {
	...
	//베이스의 기능을 확장하고 이를 설정하기 위한 setter메서드 정의
	...
	/*
	@Override : 어노테이션이라고 부르고, 오버라이딩된 메서드에 추가하여 컴파일러에게 알려주는 역할을 한다.
		필수사항은 아니므로 생략할 수 있다.
	*/
	@Override
	public void showState() {
		...
		특히 오버라이딩 된 메서드를 호출할 때 super가 없으면 재귀 호출이 되므로 주의해야한다.
		*/
		super.showState();
...
public class E06Overriding {
	...
	public static void main(String[] args) {
		...
		*/
		...
		/*
		단 부모타입의 변수로 자식 인스턴스를 참조하면 접근할 수 있는 범위가 부모 인스턴스까지로 제한되므로,
		자식쪽에 정의되어 있는 setBase()는 호출할 수 없게된다.
		*/
		speaker.setVolumn(30);
//		speaker.setBase(40); => 에러발생
		/*
		접근범위는 앞에서 설명한 바와 같이 부모까지로 제한되지만 오버라이팅 된 메서드가 있다면
		자식쪽의 메서드가 호출된다.
		*/
		speaker.showState();
	}
