...
class FriendInfoHandler {
	...
	*/
	...
	//배열에 저장된 연락처 정보를 카운트하기 위한 멤버변수로 추가할 때 마다 ++연산자로 1씩 증가한다.
	private int numOfFriends;
	
	//생성자: 인스턴스 배열의 크기를 인수로 받은 후 초기화한다.
	public FriendInfoHandler(int num) {
		//정보를 저장할 Friend타입의 인스턴스 배열을 생성한다.
	...
	public void addFriend(int choice) {
		...
		String iName,iPhone,iAddr,iNuckname,iMajor;
		...
		if(choice == 1) { //고딩
			...
			HigFriend high = new HigFriend(iName, iPhone, iAddr, iNuckname);
			/*
			참조변수를 인스턴스 배열에 추가한다.
		...
		/*
		더 이상 실행 할 문장이 없으므로 해당 메서드는 메모리에서 소멸되고,
		호출한 지점인 main 메서드로 돌아간다.
		돌아간 후 break를 만나서 switch문을 탈출해서 while문 처음으로 돌아가서 meunShow가 다시 출력된다. 
		*/
	}//end of addFriend
	...
	3. 정보 출력시 배열에 입력된 개체수만틈 반복하여 각 원소를 통해 정보를 출력한다.
	4. 출력을 위한 멤버메서드는 상속관계에서 오버라이딩 처리되어 있으므로 참조변수의
		영향을 받기 않고 항상 자식클래스에 오버라이딩된 메서드가 호출된다.
	...
	*/
	...
	public void showSimpleData() {
		...
		for(int i=0 ; i<numOfFriends ; i++) {
			/*
			만약 부모클래스인 Friend에 showBasicInfo()메서드를 정의하지 않으면 자식클래스에서는
			오버라이딩을 할 수 없으므로 개별적인 멤버메서드를 정의해야한다.
			...
			//다운 캐스팅
			 for(int i=0 ; i<numOfFriends ; i++) {
				...
				else if(myFriends[i] instanceof HigFriend) {
					((HigFriend)myFriends[i]).showBasicInfo();
				}
		 	}
		 */
			myFriends[i].showBasicInfo(); //=>오버라이딩 시 사용
		}
	...
	public void searchInfo() {
		...
		for(int i=0 ; i<numOfFriends ; i++) {
			/*
			배열의 각 인덱스에 저장된 인스턴스의 참조값을 통해 멤벼변수에 접근한다.
			검색을 위해 입력한 이름과 비교해서 일치하는 경우에만 정보를 출력한다.
			이때 compareTo 대신 equals()를 사용해도 결과는 동일하다.
			*/
	...
	public void deleteInfo() {
		...
		/*
		배열의 원소 중 삭제된 원소의 인덱스값을 저장하기 위한 변수로 삭제한 후 빈자리를 채워 넣을 때 사용한다.
		...
		*/
		...
		if(deletIndex == -1) {
			...
			*/
			...
			//삭제한 결과를 출력한다.
			System.out.println("==데이터("+ deletIndex + "번)가 삭제되었습니다.==");
		}
	}//end of deleteInfo
}
