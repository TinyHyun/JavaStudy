...
- 매개변수로 전달되는 인수는 부모타입으로 자동형변환(업캐스팅)되므로 정상적인 사용을 위해
	강제형변환(다운캐스팅) 해야하는데 이때 확인을 위해 사용한다.
*/
...
public class E09InstanceOf {
	...
	- Box타입으로 인수를 받게되면 자동형변환(업캐스팅)되므로 자식 클래스의 멤버메서드를 호출할 수 없게 된다.
	...
	*/
	...
	상속관계가 더 깊어진다면 메서드는 점점 늘어나게되어 코드는 더욱 복잡해지게 될것이다.
