...
Exception 클래스는 모든 예외를 catch할수 있으므로 반드시 마지막 블럭에 사용해야한다.
	...
	public static void main(String[] args) {
		...
		try {
			...
			for (int i=0 ; i<array.length ; i++) {
				...
				"1a"와 같이 입력하면 정수로 변경할 수 없으므로 예외가 발생한다.
				*/
			...
			//배열의 크기가 3이므로 인덱스는 2까지만 존재한다. 여기서도 예외 발생
			array[3] = age;
		}
			...
			위쪽으로 이동시키면 에러가 발생된다. 해당 클래스로 모든 예외를 catch할 수 있기때문이다.
			*/
