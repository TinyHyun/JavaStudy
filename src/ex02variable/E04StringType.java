...
public class E04StringType {
	...
	public static void main(String[] args) {
		...
		String클래스 : 참조형 변수로써 기본자료형이 아닌 클래스로 문자열을 저장하고, 조작하는 기능을 가지고 있다.
		 	Java에서는 문자열을 표현할 때 ""으로 감싸고 연결시에는 + 기호를 사용한다.
		*/
		int number;
		number = 99;
...
//		String stringNumber2 = (String)100;
		...
		/*
		문자열 + 정수
		: 우선순위가 없다면 일반적으로 문자열로 출력된다. 단, 산술연산의 결과를 출력하고 싶다면
		아래와 같이 ()로 묶어 우선순위에 대한 변화를 주면된다.
		*/
		int kor = 100, eng = 99, math = 98;
		System.out.println("총점: "+ kor + eng + math); // 총점: 1009998 형태로 출력
		System.out.println("총점: "+ (kor + eng + math)); // 덧셈연산된 결과로 총점: 297로 출력
	}
